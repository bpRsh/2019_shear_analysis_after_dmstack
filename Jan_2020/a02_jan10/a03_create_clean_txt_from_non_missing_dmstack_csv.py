...
# assertions
assert os.path.isfile('dict_flags.json')
...
idir = 'dmstack_csv{}_renamed'.format(START)
...
def func1():
    ifiles = sorted(glob.glob('{}/*_lsst_*.csv'.format(idir)))
    ifiles = [i for i in ifiles if 'lsst_mono' not in i] # lsst needs this!!!
    for ifile in ifiles:
...
def func2():
    ifiles = sorted(glob.glob('{}/*_lsst90_*.csv'.format(idir)))
    for ifile in ifiles:
        file_number = int(ifile.rstrip('.csv').split('_')[-1])
        remove_nans(ifile, file_number)

def func3():
    ifiles = sorted(glob.glob('{}/*_lsst_mono_*.csv'.format(idir)))
    for ifile in ifiles:
        file_number = int(ifile.rstrip('.csv').split('_')[-1])
        remove_nans(ifile, file_number)

def func4():
    ifiles = sorted(glob.glob('{}/*_lsst_mono90*.csv'.format(idir)))
    for ifile in ifiles:
        file_number = int(ifile.rstrip('.csv').split('_')[-1])
        remove_nans(ifile, file_number)
