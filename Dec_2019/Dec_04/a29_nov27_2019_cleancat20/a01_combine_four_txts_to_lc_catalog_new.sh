...
mkdir -p $catalogs
    ...
    for i in $(seq -f "%03g" $BEGIN $END)
        do # do of 0 to 99
            ...
            lc -C -n fN -n id -N '1 2 x' -N '1 2 errx' -N '1 2 g' -n ellip -n flux -n radius < "${L9T}".txt  | lc +all 'mag = %flux log10 -2.5 *'  |  cleancat 20  |  lc +all -r 'mag' > "${L9C}".cat

            lc -C -n fN -n id -N '1 2 x' -N '1 2 errx' -N '1 2 g' -n ellip -n flux -n radius < "${MT}".txt  |  lc +all 'mag = %flux log10 -2.5 *'  |  cleancat 20  |  lc +all -r 'mag' >  "${MC}".cat
